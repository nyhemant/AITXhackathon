...
# Kelp forest ships Junior Ranger + Park Ranger + Zoologist.
# Manta ray ships Junior Ranger + Park Ranger + Zoologist.
# Octopus ships Junior Ranger + Park Ranger + Zoologist.
...
# animal cards. Next / Print / Learn more follow ?from=<slug> (or the place-page
# referrer). Not a site-wide next-animal system. Shared african-elephant: Next:
# lion only when the parent came from Dallas (San Diego Zoo’s Start here ends on
# elephant; Safari Park’s Start here is elephant → giraffe → tiger).
...
def kit_from_query(slug: str) -> str:
    """?from= slug for Wave 1–3b zoo + Wave 4 aquarium start-here / open-card links."""
    s = (slug or "").strip()
...
def start_here_card_href(item_id: str, slug: str = "") -> str:
    """Public card href. Wave 1–4 start-here cards keep ?from={slug}."""
    iid = (item_id or "").strip()
...
def vft_tab_for_kind(venue_kind: str = "") -> str:
    """Per-place VFT tab (zoo/aquarium/parks/science) — shared by fork + SEO links."""
...
def fork_home_sub_for_kind(venue_kind: str = "") -> str:
    """Type-aware Not yet subtitle (virtual zoo / aquarium / parks / museum)."""
    tab = vft_tab_for_kind(venue_kind)
    return {
        "aquarium": "Virtual aquarium at home",
        "science": "Virtual museum at home",
        "parks": "Virtual parks at home",
        "zoo": "Virtual zoo at home",
...
def home_session_html(
    ...
    *,
    ...
    # Primary VFT CTA lives on the Not yet fork; keep #at-home for SEO/cards only.
    ...
    count_label = (
        f"{n_home} animal{plural} with talk cards"
        if n_home
        else "Talk cards for this place"
    )
    return f"""
...
def page_mission_chrome_html(
    home_href: str = "",
    *,
    home_sub: str | None = None,
    venue_kind: str = "",
    place_id: str = "",
) -> str:
    """Option A: two side-by-side choice cards (Going soon / Not yet → VFT)."""
    href = home_href or not_yet_vft_href(place_id, venue_kind)
    if home_sub is None:
        sub = fork_home_sub_for_kind(venue_kind) if (venue_kind or place_id) else FORK_HOME_SUB
    else:
        sub = home_sub
    return f"""
