...
science). Reserve for Zoologist: DNA %, Endangered
letter, subspecies, fission-fusion, bonobo split,
hunting-role detail, SIVcpz, and chromosome fusion.
Do not add Zoologist yet. Do not redo JR themes.
...
# Chimpanzee ships Junior Ranger + Park Ranger (no
# Zoologist yet).
...
TALK_ABOUT_CHIMPANZEE = (
    "Some chimps fish for termites; others crack nuts. What’s different about those tools?",
    "Why might chimps in one forest use different tool tricks than chimps somewhere else?",
    "Chimps groom each other’s fur. How could that help a friendship?",
)
PUSH_FURTHER_CHIMPANZEE = (
    "Jane Goodall first saw chimps use tools. What do you think she noticed?",
    "People and chimps can share some of the same germs. Why might that happen?",
    "Scientists later use a word like Endangered. What do you think that grade means?",
)
