...
class GiraffeZoologistStudyCardTests(unittest.TestCase):
    def test_zoologist_deck_is_answer_light(self):
        ...
        self.assertEqual(deck["source_note"], "Facts from Wikipedia, Giraffe / Reticulated giraffe. Where sources disagree on exact numbers, we keep them approximate.")
    ...
    def test_zoologist_slots_and_copy_are_locked(self):
        ...
        self.assertIn("Giraffa reticulata", correct_choice_text(questions[0]))
        self.assertIn("often four", correct_choice_text(questions[0]))
        self.assertIn("roughly double", correct_choice_text(questions[1]))
        self.assertIn("rete mirabile", correct_choice_text(questions[2]))
        self.assertIn("jugular", correct_choice_text(questions[2]))
        self.assertIn("seven cervical vertebrae", correct_choice_text(questions[3]))
        self.assertIn("greatly lengthened", correct_choice_text(questions[3]))
        self.assertIn("vomeronasal", correct_choice_text(questions[4]))
        self.assertIn("flehmen", correct_choice_text(questions[4]))
        self.assertIn("400–460", correct_choice_text(questions[5]))
        self.assertIn("usually one calf", correct_choice_text(questions[5]))
        self.assertIn("Competing browsers", correct_choice_text(questions[6]))
        self.assertIn("sexual selection", correct_choice_text(questions[6]))
        self.assertIn("sweat glands", correct_choice_text(questions[7]))
        self.assertIn("recurrent laryngeal", correct_choice_text(questions[8]).lower() + questions[8]["stem"].lower())
        self.assertIn("extraordinarily long", correct_choice_text(questions[8]))
        self.assertIn("Fission–fusion", correct_choice_text(questions[9]))
        self.assertIn("kinship", correct_choice_text(questions[9]))
    ...
    def test_zoologist_print_is_answer_light_duplex(self):
        ...
        for prompt in visible_prompts(TALK_ABOUT_GIRAFFE, "zoologist") + visible_prompts(PUSH_FURTHER_GIRAFFE, "zoologist"):
            self.assertIn(prompt, back)
        self.assertIn("Flip for answers", sheet)
        self.assertIn(WIKI_GIRAFFE, sheet)
        self.assertIn("Facts from Wikipedia, Giraffe / Reticulated giraffe. Where sources disagree on exact numbers, we keep them approximate.", sheet)
        for stem in ZOOLOGIST_STEMS:
    ...
    def test_published_artifacts_and_plumbing(self):
        ...
        print_tpl = study_print_html_for("reticulated-giraffe")
