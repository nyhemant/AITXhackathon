"""Orangutan Easy study-card: Junior Ranger teach + 10 MCQs (Park Ranger sibling)."""
...
class OrangutanEasyStudyCardTests(unittest.TestCase):
    def test_deck_is_junior_ranger_with_park_ranger_and_zoologist(self):
        ...
        self.assertEqual(deck["talk_about"], visible_prompts(TALK_ABOUT_ORANGUTAN, deck["level"]))
        self.assertEqual(deck["push_further"], visible_prompts(PUSH_FURTHER_ORANGUTAN, deck["level"]))
        ...
        self.assertIn("person of the forest", blob.lower())
        self.assertIn("malay", blob.lower())
        self.assertIn("borneo", blob.lower())
        self.assertIn("sumatra", blob.lower())
        self.assertIn("reddish-brown", blob.lower())
        self.assertIn("arms", blob.lower())
        self.assertIn("fruit", blob.lower())
        self.assertIn("solitary", blob.lower())
        self.assertIn("nest", blob.lower())
        self.assertIn("no tail", blob.lower())
        self.assertIn("great ape", blob.lower())
        self.assertIn("cheek", blob.lower())
        self.assertIn("asia", blob.lower())
        self.assertEqual(WIKI_ORANGUTAN, "https://en.wikipedia.org/wiki/Orangutan")
    ...
    def test_generator_html_is_study_not_worksheet(self):
        ...
        for prompt in visible_prompts(TALK_ABOUT_ORANGUTAN, "easy") + visible_prompts(PUSH_FURTHER_ORANGUTAN, "easy"):
    ...
    def test_other_animal_stays_generic_worksheet(self):
        ...
        self.assertNotIn("What does the name “orangutan” mean?", html)
        sea = OCTOPUS.read_text(encoding="utf-8")
        self.assertIn("card-study-pack", sea)
        self.assertNotIn("What do they eat?", sea)

    def test_published_orangutan_card_matches_easy_deck(self):
        ...
        self.assertIn("card-watch-live", main)
        self.assertIn("#habitat=orangutan", main)
        self.assertNotIn("/field-pack/virtual-zoo/?from=card#habitat=orangutan", main)
        ...
        self.assertIn('aria-label="View larger photo: Orangutan"', main)
        ...
        self.assertIn('"id": "orangutan"', html)
        ...
        for phrase in PAGE_BRITTLE:
            self.assertNotIn(phrase, main)
        print_tpl = study_print_html_for("orangutan")
        ...
        self.assertEqual(
            study_try_next_ids("orangutan"),
            ["chimpanzee", "western-lowland-gorilla", "african-lion"],
        )
    ...
    def test_print_faces_are_duplex_and_clamped(self):
        ...
        self.assertIn("/field-pack/photos/orangutan.jpg", sheet)
        ...
        self.assertIn("“Person of the forest” — from Malay", sheet)
        self.assertIn("No — orangutans are Asia’s great apes", sheet)
