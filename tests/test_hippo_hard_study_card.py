...
class HippoHardStudyCardTests(unittest.TestCase):
    def test_hard_deck_is_park_ranger_without_teach(self):
        ...
        self.assertEqual(deck["talk_about"], visible_prompts(TALK_ABOUT_HIPPO, deck["level"]))
        self.assertEqual(deck["push_further"], visible_prompts(PUSH_FURTHER_HIPPO, deck["level"]))
    ...
    def test_hard_slots_and_copy_are_locked(self):
        ...
        self.assertIn("walk or bounce", correct_choice_text(questions[0]))
        self.assertIn("debated", correct_choice_text(questions[0]).lower())
        self.assertIn("stretch of water", correct_choice_text(questions[1]))
        self.assertIn("little territoriality", correct_choice_text(questions[1]))
        self.assertIn("pods", correct_choice_text(questions[2]))
        self.assertIn("graze alone", correct_choice_text(questions[2]))
        self.assertIn("wheeze-honk", correct_choice_text(questions[3]))
        self.assertIn("long way", correct_choice_text(questions[3]))
        self.assertIn("elephants and rhinos", correct_choice_text(questions[4]))
        self.assertIn("next-largest", correct_choice_text(questions[4]))
        self.assertIn("sunscreen", correct_choice_text(questions[5]))
        self.assertIn("microbes", correct_choice_text(questions[5]))
        self.assertIn("breathe regularly", correct_choice_text(questions[6]))
        self.assertIn("submerged rest", correct_choice_text(questions[6]))
        self.assertIn("Rivers, lakes, and mangrove swamps", correct_choice_text(questions[7]))
        self.assertIn("sub-Saharan Africa", correct_choice_text(questions[7]))
        self.assertIn("Habitat loss", correct_choice_text(questions[8]))
        self.assertIn("meat and teeth", correct_choice_text(questions[8]))
        self.assertIn("rides on her back", correct_choice_text(questions[9]))
    ...
    def test_hard_print_is_answer_light_duplex(self):
        ...
        for prompt in visible_prompts(TALK_ABOUT_HIPPO, "hard") + visible_prompts(PUSH_FURTHER_HIPPO, "hard"):
    ...
    def test_other_animal_decks_untouched(self):
        ...
        self.assertEqual(tortoise_hard["talk_about"], visible_prompts(TALK_ABOUT_TORTOISE, tortoise_hard["level"]))
        self.assertEqual(tortoise_hard["push_further"], visible_prompts(PUSH_FURTHER_TORTOISE, tortoise_hard["level"]))
        ...
        self.assertEqual(zebra_hard["talk_about"], visible_prompts(TALK_ABOUT_ZEBRA, zebra_hard["level"]))
        self.assertEqual(zebra_hard["push_further"], visible_prompts(PUSH_FURTHER_ZEBRA, zebra_hard["level"]))
    ...
    def test_published_artifacts_and_plumbing(self):
        ...
        self.assertIn("zoologist", payload["nile-hippo"]["levels"])
        self.assertEqual(payload["nile-hippo"]["levels"]["zoologist"]["teach"], [])
        ...
        print_tpl = study_print_html_for("nile-hippo")
