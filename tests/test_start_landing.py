...
class StartLandingTests(unittest.TestCase):
    ...
    def test_route_is_start_not_home(self):
        ...
        self.assertIn("Turn the zoo into an expedition.", body)
        ...
        self.assertIsNotNone(_safe_start_path("/start/home-print-table-desktop.jpg"))
        ...
        self.assertIn("KidZooKit — Free At-Home Zoo for Kids", about.wfile.getvalue().decode("utf-8"))
        ...
        desktop_still = _get("/start/home-print-table-desktop.jpg")
        self.assertEqual(desktop_still._code, 200)
        self.assertTrue(desktop_still.wfile.getvalue().startswith(b"\xff\xd8"))
    ...
    def test_hero_is_world_map_with_animal_hotspots(self):
        ...
        self.assertIn("Turn the zoo into an expedition.", chapter)
        ...
        self.assertNotIn(".start-hero-hotspot:hover", self.css)
        self.assertNotIn("rgba(255, 252, 247, 0.16)", self.css)
    ...
    def test_awake_line_deep_links_named_animal(self):
        """Awake chapter is flamingo-first rotator; hit deep-links the active animal."""
        ...
        self.assertIn('<span class="sr-only">A flamingo&apos;s awake. Look!</span>', chapter)
        self.assertIn("data-wildlife-rotator", chapter)
        self.assertIn("start-wildlife-word", chapter)
        self.assertIn('data-sizer="flamingo"', chapter)
        self.assertIn(
            'class="start-wildlife-word is-active" data-habitat="caribbean-flamingo">flamingo</span>',
            chapter,
        )
        self.assertIn("Look!", chapter)
        self.assertNotIn("A flamingo&apos;s awake. Look.</span>", chapter)
        words = re.findall(
        ...
        got = {hid: word for hid, word in words}
        for hid, word in expected.items():
            self.assertEqual(got.get(hid), word, hid)
        self.assertNotIn("nile-hippo", chapter)
        self.assertNotIn(">hippo<", chapter)
        js = (START / "start.js").read_text(encoding="utf-8")
        self.assertIn("syncWildlifeHit", js)
        self.assertIn('bootRotator("[data-wildlife-rotator]"', js)
    ...
    def test_home_chapter_is_local_print_table(self):
        ...
        self.assertIn("/start/home-print-table-desktop-640.jpg 640w", chapter)
        self.assertIn("/start/home-print-table-desktop-1280.jpg 1280w", chapter)
        self.assertIn("/start/home-print-table-desktop.jpg 1792w", chapter)
        ...
        self.assertTrue((START / "home-print-table-desktop.jpg").is_file())
        self.assertTrue((START / "home-print-table-desktop-1280.jpg").is_file())
        self.assertTrue((START / "home-print-table-desktop-640.jpg").is_file())
        self.assertTrue((START / "home-print-table-desktop.jpg").read_bytes().startswith(b"\xff\xd8"))
        ...
        self.assertIn("object-position: 45% 50%", self.css)
        self.assertIn("object-position: 12% 22%", self.css)
        self.assertNotIn("object-position: 36% 24%", self.css)
        self.assertIn("framePlate", self.js)
        self.assertIn("fitHotboxPosition", self.js)
        self.assertIn("isMobilePortrait", self.js)
        self.assertIn("HOTBOX_MOBILE", self.js)
        self.assertIn("HOTBOX_DESKTOP", self.js)
        self.assertIn("currentHotbox", self.js)
        self.assertIn('matchMedia("(min-width: 641px)")', self.js)
        self.assertIn(".start-home-screen", self.css)
        self.assertIn("8.20% 20.57%", self.css)
        self.assertIn("12.33% 11.47%", self.css)
        self.assertIn("48.89% 47.13%", self.css)
        self.assertNotIn("8.33% 20.63%", self.css)
    ...
    def test_going_chapter_is_local_jpeg(self):
        ...
        self.assertNotIn("start-kicker", chapter)
        self.assertIn("Put the", chapter)
        self.assertIn("day in their hands.", chapter)
        ...
        self.assertNotIn("srcset=", hit.group(0))
        self.assertNotIn("data-srcset=", hit.group(0))
        ...
        secondary = re.search(
            r'<a class="start-going-secondary" href="([^"]+)">([^<]+)</a>',
            chapter,
        )
        self.assertIsNotNone(secondary)
        self.assertEqual(secondary.group(1), "/field-pack/dallas-zoo/")
        self.assertEqual(secondary.group(2), "Sample visit")
        self.assertLess(chapter.find("Explore Places Near You"), chapter.find(">Sample visit<"))
        ...
        self.assertIn('data-src="/start/going-georgia-aquarium.jpg?v=3"', chapter)
        ...
        self.assertIn('data-src="/start/going-san-diego-zoo.jpg?v=2"', chapter)
        ...
        loaded_going = [
            tag
            for tag in re.findall(r"<img\b[\s\S]*?>", chapter)
            if re.search(r"\ssrc=", tag)
        ]
        self.assertEqual(len(loaded_going), 1, loaded_going)
        self.assertIn('src="/start/going-giraffe.jpg"', loaded_going[0])
        self.assertNotIn("srcset=", loaded_going[0])
        self.assertEqual(len(re.findall(r'\ssrc="/start/going-giraffe.jpg"', chapter)), 1)
        self.assertEqual(len(re.findall(r'\ssrc="/start/going-georgia-aquarium.jpg', chapter)), 0)
        self.assertEqual(len(re.findall(r'\ssrc="/start/going-san-diego-zoo.jpg', chapter)), 0)
        self.assertGreaterEqual(chapter.count("data-src="), 6)
        self.assertIn("function armPlate", self.js)
        self.assertIn("function armGoingPlates()", self.js)
        self.assertIn("armGoingPlates()", self.js)
        self.assertIn(
        ...
        self.assertIn(".start-going-panel img:not([src])", self.css)
        self.assertIn(".start-going-slide img:not([src])", self.css)
        self.assertIn("#start-teach .start-chapter-still:not([src])", self.css)
        self.assertIn(".start-teach-slide img:not([src])", self.css)
        ...
        secondary_css = re.search(
            r"#start-going \.start-going-secondary \{([^}]+)\}",
            self.css,
        )
        self.assertIsNotNone(secondary_css)
        self.assertIn("text-decoration: underline", secondary_css.group(1))
        self.assertIn("background: transparent", secondary_css.group(1))
        self.assertNotIn("border-radius: 999px", secondary_css.group(1))
        self.assertNotIn("webgl", self.css.lower())
        self.assertNotIn("parallax", self.css.lower())
    ...
    def test_teach_chapter_is_local_jpeg(self):
        ...
        self.assertIn('data-src="/start/teach-lion.jpg"', chapter)
        self.assertIn('data-src="/start/teach-elephant.jpg"', chapter)
        self.assertIn('data-src="/start/teach-giraffe.jpg"', chapter)
        ...
        self.assertEqual(loaded_teach, [])
        self.assertEqual(len(re.findall(r'\ssrc="/start/teach-', chapter)), 0)
        self.assertIn("function armTeachPlates()", self.js)
        self.assertIn("isMobileTeach()", self.js)
    ...
    def test_locked_headline_and_no_marketing_stats(self):
        ...
        self.assertNotIn(
            "Turn the zoo, aquarium, museum, or park into an expedition.",
            self.html,
        )
        self.assertIn("Put the zoo day in their hands.", self.html)
        self.assertNotIn(
        ...
        self.assertIn(".start-wildlife-word", self.css)
        ...
        self.assertIn(
            ".start-going-venue-word:not(.is-active) {\n  display: none;\n}",
            self.css,
        )
        ...
        self.assertIn('class="start-venue-sizer" data-sizer="Zoo"></span>', self.html)
        self.assertIn("transition: width 250ms ease", self.css)
        self.assertIn("fitSlot", self.js)
        self.assertIn('setAttribute("data-sizer"', self.js)
        self.assertNotIn("sizer.textContent", self.js)
        self.assertIn(".start-venue-sizer::before", self.css)
        self.assertIn("content: attr(data-sizer)", self.css)
        self.assertIn("wordWidth", self.js)
        self.assertIn("<title>Turn the zoo into an expedition. · KidZooKit</title>", self.html)
        self.assertIn(
            '<meta property="og:title" content="Turn the zoo into an expedition. · KidZooKit" />',
            self.html,
        )
        self.assertIn(
            '<meta name="twitter:title" content="Turn the zoo into an expedition. · KidZooKit" />',
            self.html,
        )
    ...
    def test_chapter_pills_are_locked_and_under_the_box(self):
        ...
        going = chapters["start-going"].group(0)
        secondary = re.search(
            r'<a class="start-going-secondary" href="([^"]+)">([^<]+)</a>',
            going,
        )
        self.assertIsNotNone(secondary)
        self.assertEqual(
            (secondary.group(1), secondary.group(2)),
            ("/field-pack/dallas-zoo/", "Sample visit"),
        )
        self.assertEqual(self.html.count('class="start-going-secondary"'), 1)
    ...
    def test_mobile_scrub_keeps_thumb_targets(self):
        ...
        self.assertIn("left: 8%", block)
        self.assertIn("top: 58%", block)
    ...
    def test_motion_is_optional_and_tappable(self):
        ...
        self.assertIn('start.js?v=39', self.html)
        self.assertIn("start.css?v=76", self.html)
