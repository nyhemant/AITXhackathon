...
class WarthogEasyStudyCardTests(unittest.TestCase):
    def test_deck_is_junior_ranger_with_park_ranger_and_zoologist(self):
        ...
        self.assertEqual(deck["talk_about"], visible_prompts(TALK_ABOUT_WARTHOG, deck["level"]))
        self.assertEqual(deck["push_further"], visible_prompts(PUSH_FURTHER_WARTHOG, deck["level"]))
    ...
    def test_generator_html_is_study_not_worksheet(self):
        ...
        for prompt in visible_prompts(TALK_ABOUT_WARTHOG, "easy") + visible_prompts(PUSH_FURTHER_WARTHOG, "easy"):
        ...
        self.assertIn("Facts from Wikipedia, Common warthog. Where sources disagree on exact numbers, we keep them approximate.", html)
    ...
    def test_published_warthog_card_matches_easy_deck(self):
        ...
        for phrase in PAGE_BRITTLE:
            self.assertNotIn(phrase, main)
        print_tpl = study_print_html_for("warthog")
