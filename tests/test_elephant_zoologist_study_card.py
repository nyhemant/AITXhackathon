...
class ElephantZoologistStudyCardTests(unittest.TestCase):
    ...
    def test_zoologist_slots_and_copy_are_locked(self):
        ...
        self.assertIn("Loxodonta africana", correct_choice_text(questions[0]))
        self.assertIn("L. cyclotis", correct_choice_text(questions[0]))
        self.assertIn("IUCN", questions[0]["why"])
        self.assertIn("upper lip", correct_choice_text(questions[1]))
        self.assertIn("nose", correct_choice_text(questions[1]))
        self.assertIn("two finger-like", correct_choice_text(questions[2]))
        self.assertIn("Asian", correct_choice_text(questions[2]))
        self.assertIn("ground-borne", correct_choice_text(questions[3]))
        self.assertIn("feet", correct_choice_text(questions[3]))
        self.assertIn("trunk", correct_choice_text(questions[3]))
        self.assertIn("testosterone", correct_choice_text(questions[4]).lower())
        self.assertIn("temporal", correct_choice_text(questions[4]).lower())
        self.assertIn("allomother", correct_choice_text(questions[5]).lower())
        self.assertIn("starve", correct_choice_text(questions[6]).lower())
        self.assertIn("hindgut", correct_choice_text(questions[7]).lower())
        self.assertIn("TP53", correct_choice_text(questions[8]))
        self.assertIn("Peto", correct_choice_text(questions[8]))
        self.assertIn("seed", correct_choice_text(questions[9]).lower())
        self.assertIn("dung", correct_choice_text(questions[9]).lower())
    ...
    def test_zoologist_print_is_answer_light_duplex(self):
        ...
        for prompt in visible_prompts(TALK_ABOUT_ELEPHANT, "zoologist") + visible_prompts(PUSH_FURTHER_ELEPHANT, "zoologist"):
