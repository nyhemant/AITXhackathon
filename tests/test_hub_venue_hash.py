...
class HubVenueHashTests(unittest.TestCase):
    ...
    def test_dead_places_hero_and_outing_css_are_gone(self):
        html = (FP / "index.html").read_text(encoding="utf-8")
        hook = (FP / "js" / "landing-hook.js").read_text(encoding="utf-8")
        map_js = MAP_JS.read_text(encoding="utf-8")
        landing_css = (FP / "css" / "landing.css").read_text(encoding="utf-8")
        styles = (FP / "css" / "styles.css").read_text(encoding="utf-8")
        self.assertNotIn('id="continue-chip"', html)
        self.assertNotIn('id="waiting-cities"', html)
        self.assertIn('id="hero-place-search"', html)
        self.assertIn("location.replace(\"/field-pack/\" + encodeURIComponent(id) + \"/\")", html)
        self.assertNotIn("hero-mode-btn", hook)
        self.assertNotIn("hero-search-mode-hint", hook)
        self.assertNotIn("hero-moment-link", hook)
        self.assertNotIn("1less-babys-day-out-trips-v1", hook)
        self.assertNotIn("arya-field-pack-trips-v2", hook)
        self.assertNotIn("1less-saved-cities", hook)
        self.assertIn("function wireHeroSearch(", hook)
        self.assertIn("goPlace(", hook)
        self.assertNotIn('id === "app.html"', map_js)
        self.assertNotIn(".hero-mode-btn", landing_css)
        self.assertNotIn(".hero-moment-strip", landing_css)
        self.assertNotIn(".hero-search-mode-hint", landing_css)
        self.assertNotIn(".ready-grid", landing_css)
        self.assertNotIn(".continue-chip", landing_css)
        self.assertNotIn(".waiting-cities", landing_css)
        self.assertNotIn(".app-outing", styles)
        self.assertNotIn(".app-nav", styles)
        self.assertNotIn(".outing-hero", styles)
