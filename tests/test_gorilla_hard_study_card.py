...
class GorillaHardStudyCardTests(unittest.TestCase):
    ...
    def test_hard_print_is_answer_light_duplex(self):
        ...
        for prompt in visible_prompts(TALK_ABOUT_GORILLA, "hard") + visible_prompts(PUSH_FURTHER_GORILLA, "hard"):
            self.assertIn(prompt, back)
        self.assertIn("Flip for answers", sheet)
        self.assertIn(WIKI_WESTERN_LOWLAND_GORILLA, sheet)
        self.assertIn("Facts from Wikipedia, Western lowland gorilla. Where sources disagree on exact numbers, we keep them approximate.", sheet)
        for stem in HARD_STEMS:
    ...
    def test_other_animal_decks_untouched(self):
        ...
        self.assertEqual(hippo_hard["talk_about"], visible_prompts(TALK_ABOUT_HIPPO, hippo_hard["level"]))
        self.assertEqual(hippo_hard["push_further"], visible_prompts(PUSH_FURTHER_HIPPO, hippo_hard["level"]))
        ...
        self.assertEqual(tiger_hard["talk_about"], visible_prompts(TALK_ABOUT_TIGER, tiger_hard["level"]))
        self.assertEqual(tiger_hard["push_further"], visible_prompts(PUSH_FURTHER_TIGER, tiger_hard["level"]))
