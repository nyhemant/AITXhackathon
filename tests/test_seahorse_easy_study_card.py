"""Seahorse Easy study-card: Junior Ranger teach + 10 MCQs (no Park Ranger yet)."""
...
class SeahorseEasyStudyCardTests(unittest.TestCase):
    def test_deck_is_junior_ranger_only(self):
        ...
        self.assertEqual(shipped_levels_for("seahorse"), ("easy",))
        self.assertIsNone(study_deck_for("seahorse", "hard"))
        self.assertIsNone(study_deck_for("seahorse", "zoologist"))
    ...
    def test_artifacts_include_seahorse_easy_only(self):
        ...
        self.assertEqual(set(horse["levels"]), {"easy"})
        self.assertNotIn("hard", horse["levels"])
        self.assertNotIn("zoologist", horse["levels"])
