...
class GiantPandaEasyStudyCardTests(unittest.TestCase):
    def test_deck_is_junior_ranger_with_park_ranger_and_zoologist(self):
        ...
        self.assertEqual(deck["talk_about"], visible_prompts(TALK_ABOUT_GIANT_PANDA, deck["level"]))
        self.assertEqual(deck["push_further"], visible_prompts(PUSH_FURTHER_GIANT_PANDA, deck["level"]))
    ...
    def test_generator_html_is_study_not_worksheet(self):
        ...
        for prompt in visible_prompts(TALK_ABOUT_GIANT_PANDA, "easy") + visible_prompts(PUSH_FURTHER_GIANT_PANDA, "easy"):
    ...
    def test_published_giant_panda_card_matches_easy_deck(self):
        ...
        self.assertIn("/field-pack/virtual-field-trip/?tab=zoo&from=card#habitat=giant-panda", main.replace("&amp;", "&"))
        ...
        self.assertIn('aria-label="View larger photo: Giant panda"', main)
        ...
        for phrase in PAGE_BRITTLE:
            self.assertNotIn(phrase, main)
        print_tpl = study_print_html_for("giant-panda")
