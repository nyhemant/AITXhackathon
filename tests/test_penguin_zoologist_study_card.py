...
class PenguinZoologistStudyCardTests(unittest.TestCase):
    def test_zoologist_deck_is_answer_light(self):
        ...
        self.assertEqual(deck["source_note"], "Facts from Wikipedia, African penguin. Where sources disagree on exact numbers, we keep them approximate.")
    ...
    def test_zoologist_slots_and_copy_are_locked(self):
        ...
        self.assertIn("Spheniscus", correct_choice_text(questions[0]))
        self.assertIn("Humboldt", correct_choice_text(questions[0]))
        self.assertIn("Magellanic", correct_choice_text(questions[0]))
        self.assertIn("Galápagos", correct_choice_text(questions[0]))
        self.assertIn("African continent", correct_choice_text(questions[1]))
        self.assertIn("Old World", correct_choice_text(questions[1]))
        self.assertIn("excretes concentrated salt", correct_choice_text(questions[2]))
        self.assertIn("supraorbital", questions[2]["stem"].lower())
        self.assertIn("osmotic", questions[2]["stem"].lower() + questions[2]["why"].lower())
        self.assertIn("Classic countershading", correct_choice_text(questions[3]))
        self.assertIn("vasodilation", correct_choice_text(questions[4]))
        self.assertIn("vasoconstriction", correct_choice_text(questions[4]))
        self.assertIn("Prey shortage", correct_choice_text(questions[5]))
        self.assertIn("habitat loss", correct_choice_text(questions[5]))
        self.assertIn("Purse-seine", correct_choice_text(questions[6]))
        self.assertIn("sardines and anchovies", correct_choice_text(questions[6]))
        self.assertIn("foraging range", correct_choice_text(questions[7]))
        self.assertIn("pursuit diving", correct_choice_text(questions[8]))
        self.assertIn("relatively shallow", correct_choice_text(questions[8]))
        self.assertIn("Tubenoses", correct_choice_text(questions[9]))
        self.assertIn("albatrosses", correct_choice_text(questions[9]))
        self.assertIn("petrels", correct_choice_text(questions[9]))
    ...
    def test_zoologist_print_is_answer_light_duplex(self):
        ...
        for prompt in visible_prompts(TALK_ABOUT_PENGUIN, "zoologist") + visible_prompts(PUSH_FURTHER_PENGUIN, "zoologist"):
            self.assertIn(prompt, back)
        self.assertIn("Flip for answers", sheet)
        self.assertIn(WIKI_AFRICAN_PENGUIN, sheet)
        self.assertIn("Facts from Wikipedia, African penguin. Where sources disagree on exact numbers, we keep them approximate.", sheet)
        for stem in ZOOLOGIST_STEMS:
