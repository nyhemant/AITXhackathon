...
class ZebraZoologistStudyCardTests(unittest.TestCase):
    def test_zoologist_deck_is_answer_light(self):
        ...
        self.assertEqual(deck["talk_about"], visible_prompts(TALK_ABOUT_ZEBRA, deck["level"]))
        self.assertEqual(deck["push_further"], visible_prompts(PUSH_FURTHER_ZEBRA, deck["level"]))
    ...
    def test_zoologist_slots_and_copy_are_locked(self):
        ...
        self.assertIn("Equus quagga", correct_choice_text(questions[0]))
        self.assertIn("burchellii", correct_choice_text(questions[0]))
        self.assertIn("three living", correct_choice_text(questions[1]).lower())
        self.assertIn("Grévy", correct_choice_text(questions[1]))
        self.assertIn("mountain zebra", correct_choice_text(questions[1]).lower())
        self.assertIn("close relationship", correct_choice_text(questions[2]).lower())
        self.assertIn("species complex", correct_choice_text(questions[2]).lower())
        self.assertIn("subspecies forever", questions[2]["why"].lower())
        self.assertIn("lessen toward the south", correct_choice_text(questions[3]).lower())
        self.assertIn("shadow", correct_choice_text(questions[3]).lower())
        self.assertIn("cline", questions[3]["why"].lower())
        self.assertIn("experimental", questions[4]["stem"].lower())
        self.assertIn("fly deterrence", correct_choice_text(questions[4]).lower())
        self.assertIn("debated", correct_choice_text(questions[4]).lower())
        self.assertIn("camouflage", correct_choice_text(questions[4]).lower())
        self.assertIn("flehmen", correct_choice_text(questions[5]).lower())
        self.assertIn("vomeronasal", correct_choice_text(questions[5]).lower())
        self.assertIn("gradually", correct_choice_text(questions[6]).lower())
        self.assertIn("mare membership", correct_choice_text(questions[6]).lower())
        self.assertIn("even the stallion", correct_choice_text(questions[7]).lower())
        self.assertIn("Memory of places", correct_choice_text(questions[8]))
        self.assertIn("not only day-to-day", correct_choice_text(questions[8]).lower())
        self.assertIn("Hunting and habitat", correct_choice_text(questions[9]))
        self.assertIn("Near Threatened", correct_choice_text(questions[9]))
        self.assertIn("snapshot", correct_choice_text(questions[9]).lower() + questions[9]["why"].lower())
    ...
    def test_zoologist_print_is_answer_light_duplex(self):
        ...
        for prompt in visible_prompts(TALK_ABOUT_ZEBRA, "zoologist") + visible_prompts(PUSH_FURTHER_ZEBRA, "zoologist"):
