...
class GorillaEasyStudyCardTests(unittest.TestCase):
    def test_deck_is_junior_ranger_with_park_ranger_sibling(self):
        ...
        self.assertEqual(deck["talk_about"], visible_prompts(TALK_ABOUT_GORILLA, deck["level"]))
        self.assertEqual(deck["push_further"], visible_prompts(PUSH_FURTHER_GORILLA, deck["level"]))
    ...
    def test_generator_html_is_study_not_worksheet(self):
        ...
        for prompt in visible_prompts(TALK_ABOUT_GORILLA, "easy") + visible_prompts(PUSH_FURTHER_GORILLA, "easy"):
    ...
    def test_published_gorilla_card_matches_easy_deck(self):
        ...
        self.assertIn("/field-pack/virtual-field-trip/?tab=zoo&from=card#habitat=western-lowland-gorilla", main.replace("&amp;", "&"))
        ...
        self.assertIn('aria-label="View larger photo: Western lowland gorilla"', main)
