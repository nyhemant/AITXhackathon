...
class CardsExplorerTests(unittest.TestCase):
    ...
    def test_cards_route_200_and_find_a_card(self):
        ...
        self.assertIn("Animal cards", body)
    ...
    def test_play_door_is_first_and_teaches_print_cut_hide(self):
        ...
        self.assertIn("Animal cards", door)
        self.assertNotIn("Hide-and-seek at home", door)
        self.assertNotIn("cards-play-kicker", door)
        ...
        self.assertNotIn(">Find card</button>", self.html)
        self.assertNotIn("ready to print, cut, and hide", door)
    ...
    def test_explorer_is_samples_not_a_58_card_wall(self):
        ...
        self.assertNotIn("All 42 cards", self.html)
        ...
        self.assertEqual(self.html.count('class="cards-hub-item"'), 57)
        ...
        self.assertNotIn("Experimental · Quiet", self.html)
        ...
        self.assertNotIn("cards-theme-parks", self.html)
        self.assertIn("cards-theme-attractions", self.html)
        self.assertIn("cards-experimental", self.html)
        self.assertEqual(self.html.count("cards-accordion-panel"), 3)
        css = (REPO / "static" / "field-pack" / "css" / "landing.css").read_text(encoding="utf-8")
        self.assertIn(".cards-explorer .cards-experimental {", css)
        self.assertIn("background: #f4f2ee;", css)
        ...
        self.assertEqual(self.html.count('Wildlife <span class="seo-dir-count">25</span>'), 1)
        self.assertIn('data-card-accordion="wildlife" open', self.html)
        self.assertIn('data-card-accordion="sealife"', self.html)
        self.assertNotIn('data-card-accordion="parks"', self.html)
        ...
        self.assertNotIn('data-card-id="cuyahoga-towpath"', primary)
        self.assertNotIn('data-card-id="sci-dinosaur"', primary)
        for cid in ("american-bison", "american-alligator", "elk"):
            li = [line for line in primary.splitlines() if f'data-card-id="{cid}"' in line and "cards-hub-item" in line][0]
            self.assertIn('data-card-group="wildlife"', li, cid)
            self.assertIn('data-card-kind="animal"', li, cid)
        ...
        self.assertIn("isExperimental", explorer_js)
        self.assertIn('hash === "cards-attractions" || hash === "cards-experimental" || hash === "cards-beta"', explorer_js)
        self.assertNotIn("cards-parks", explorer_js)
        self.assertIn('PRIMARY_HUB_SECTION_IDS = ("wildlife", "sealife")', gen)
        self.assertIn("EXPERIMENTAL_SHELF_LABEL = \"Beta\"", gen)
        self.assertIn("Museum stops &amp; extras", gen)
        self.assertNotIn("Museum &amp; science cards", gen)
        self.assertNotIn('id="cards-parks"', gen)
        self.assertNotIn("cards-parks-wrap", gen)
