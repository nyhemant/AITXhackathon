...
class EelEasyStudyCardTests(unittest.TestCase):
    def test_deck_is_junior_ranger_only(self):
        ...
        self.assertEqual(deck["talk_about"], visible_prompts(TALK_ABOUT_EEL, deck["level"]))
        self.assertEqual(deck["push_further"], visible_prompts(PUSH_FURTHER_EEL, deck["level"]))
    ...
    def test_generator_html_is_study_not_worksheet(self):
        ...
        for prompt in visible_prompts(TALK_ABOUT_EEL, "easy") + visible_prompts(PUSH_FURTHER_EEL, "easy"):
    ...
    def test_published_eel_card_matches_easy_deck(self):
        ...
        for phrase in PAGE_BRITTLE:
            self.assertNotIn(phrase, main)
        print_tpl = study_print_html_for("eel")
