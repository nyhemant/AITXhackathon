...
class WebApiScenarioTest(unittest.TestCase):
    ...
    def test_web_page_exposes_dinner_only_public_alpha(self):
        ...
        self.assertIn('article.className = "dinner-card"', HTML)
        self.assertIn("function parseDinnerMessage(message)", HTML)
        self.assertIn('message.split("\\n")', HTML)
        self.assertIn("function renderDinnerCard(response)", HTML)
        ...
        self.assertIn("renderDinnerCard(response);", HTML)
        self.assertIn('addBubble("agent", response.message);', HTML)
