...
class CheetahEasyStudyCardTests(unittest.TestCase):
    def test_deck_is_junior_ranger_with_park_ranger_and_zoologist(self):
        ...
        self.assertEqual(deck["talk_about"], visible_prompts(TALK_ABOUT_CHEETAH, deck["level"]))
        self.assertEqual(deck["push_further"], visible_prompts(PUSH_FURTHER_CHEETAH, deck["level"]))
    ...
    def test_generator_html_is_study_not_worksheet(self):
        ...
        for prompt in visible_prompts(TALK_ABOUT_CHEETAH, "easy") + visible_prompts(PUSH_FURTHER_CHEETAH, "easy"):
    ...
    def test_published_cheetah_card_matches_easy_deck(self):
        ...
        self.assertIn("card-watch-live", main)
        self.assertIn("#habitat=cheetah", main)
        self.assertNotIn("/field-pack/virtual-zoo/?from=card#habitat=cheetah", main)
        ...
        self.assertIn('aria-label="View larger photo: Cheetah"', main)
        ...
        for phrase in BRITTLE:
            self.assertNotIn(phrase, main)
        print_tpl = study_print_html_for("cheetah")
