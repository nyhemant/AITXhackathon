...
class StudyCardAnswerKeyTests(unittest.TestCase):
    def test_traffic_set_is_fourteen_three_level_animals_plus_orangutan_jr_pr(self):
    ...
    def test_overall_correct_b_is_not_a_majority(self):
        ...
        self.assertEqual(len(letters), 440)
        share_b = letters.count("B") / len(letters)
        self.assertLessEqual(
            share_b,
            MAX_OVERALL_B_SHARE,
            f"correct==B is {share_b:.1%} ({letters.count('B')}/440)",
        )
        for letter in LETTERS:
