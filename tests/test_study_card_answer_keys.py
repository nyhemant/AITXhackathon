...
class StudyCardAnswerKeyTests(unittest.TestCase):
    def test_traffic_set_is_seventeen_three_level_animals_plus_ostrich_jr(self):
    ...
    def test_overall_correct_b_is_not_a_majority(self):
        ...
        self.assertEqual(len(letters), 520)
        share_b = letters.count("B") / len(letters)
        self.assertLessEqual(
            share_b,
            MAX_OVERALL_B_SHARE,
            f"correct==B is {share_b:.1%} ({letters.count('B')}/520)",
        )
        for letter in LETTERS:
