...
class StudyCardAnswerKeyTests(unittest.TestCase):
    def test_traffic_set_is_fifteen_three_level_animals_plus_giant_panda_jr_pr(self):
    ...
    def test_overall_correct_b_is_not_a_majority(self):
        ...
        self.assertEqual(len(letters), 470)
        share_b = letters.count("B") / len(letters)
        self.assertLessEqual(
            share_b,
            MAX_OVERALL_B_SHARE,
            f"correct==B is {share_b:.1%} ({letters.count('B')}/470)",
        )
        for letter in LETTERS:
