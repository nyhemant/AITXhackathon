...
class FlamingoHardStudyCardTests(unittest.TestCase):
    def test_hard_deck_is_park_ranger_without_teach(self):
        ...
        self.assertEqual(deck["talk_about"], visible_prompts(TALK_ABOUT_FLAMINGO, deck["level"]))
        self.assertEqual(deck["push_further"], visible_prompts(PUSH_FURTHER_FLAMINGO, deck["level"]))
    ...
    def test_hard_slots_and_copy_are_locked(self):
        ...
        self.assertIn("Carotenoid pigments", correct_choice_text(questions[0]))
        self.assertIn("algae, shrimp", correct_choice_text(questions[0]))
        self.assertIn("Upside down", correct_choice_text(questions[1]))
        self.assertIn("inverted", correct_choice_text(questions[1]))
        self.assertIn("Strain tiny food", correct_choice_text(questions[2]))
        self.assertIn("crop milk", correct_choice_text(questions[3]))
        self.assertIn("Both parents", correct_choice_text(questions[3]))
        self.assertIn("Typically one egg", correct_choice_text(questions[4]))
        self.assertIn("passive joint lock", correct_choice_text(questions[5]))
        self.assertIn("almost no muscle", correct_choice_text(questions[5]))
        self.assertIn("salty or alkaline", correct_choice_text(questions[6]))
        self.assertIn("most vividly coloured", correct_choice_text(questions[7]))
        self.assertIn("head-flagging", correct_choice_text(questions[8]))
        self.assertIn("wing salutes", correct_choice_text(questions[8]))
        self.assertIn("carotenoids in its diet", correct_choice_text(questions[9]))
        self.assertIn("feeding a chick", correct_choice_text(questions[9]))
    ...
    def test_hard_print_is_answer_light_duplex(self):
        ...
        for prompt in visible_prompts(TALK_ABOUT_FLAMINGO, "hard") + visible_prompts(PUSH_FURTHER_FLAMINGO, "hard"):
    ...
    def test_published_artifacts_and_plumbing(self):
        ...
        print_tpl = study_print_html_for("caribbean-flamingo")
