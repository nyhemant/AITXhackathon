...
class PenguinHardStudyCardTests(unittest.TestCase):
    ...
    def test_hard_slots_and_copy_are_locked(self):
        ...
        self.assertIn("Lose heat", correct_choice_text(questions[0]))
        self.assertIn("flushes pinker", correct_choice_text(questions[0]))
        self.assertIn("unique, like a fingerprint", correct_choice_text(questions[1]))
        self.assertIn("all its feathers at once", correct_choice_text(questions[2]))
        self.assertIn("stay out of the water for weeks", correct_choice_text(questions[2]))
        self.assertIn("salt gland near the eye", correct_choice_text(questions[3]))
        self.assertIn("Not enough prey fish", correct_choice_text(questions[4]))
        self.assertIn("commercial fishing", correct_choice_text(questions[4]))
        self.assertIn("warming seas", correct_choice_text(questions[4]))
        self.assertIn("dung layer", correct_choice_text(questions[5]))
        self.assertIn("nest burrows", correct_choice_text(questions[5]))
        self.assertIn("long-term", correct_choice_text(questions[6]))
        self.assertIn("same nest site", correct_choice_text(questions[6]))
        self.assertIn("coasts and islands of southern Africa", correct_choice_text(questions[7]))
        self.assertIn("Spheniscus", correct_choice_text(questions[8]))
        self.assertIn("dark band", correct_choice_text(questions[8]))
        self.assertIn("opposite ends of the planet", correct_choice_text(questions[9]))
    ...
    def test_hard_print_is_answer_light_duplex(self):
        ...
        for prompt in visible_prompts(TALK_ABOUT_PENGUIN, "hard") + visible_prompts(PUSH_FURTHER_PENGUIN, "hard"):
