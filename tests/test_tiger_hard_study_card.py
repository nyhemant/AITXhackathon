...
class TigerHardStudyCardTests(unittest.TestCase):
    ...
    def test_hard_print_is_answer_light_duplex(self):
        ...
        for prompt in visible_prompts(TALK_ABOUT_TIGER, "hard") + visible_prompts(PUSH_FURTHER_TIGER, "hard"):
            self.assertIn(prompt, back)
        self.assertIn("Flip for answers", sheet)
        self.assertIn(WIKI_SUMATRAN_TIGER, sheet)
        self.assertIn("Facts from Wikipedia, Sumatran tiger. Where sources disagree on exact numbers, we keep them approximate.", sheet)
        for stem in HARD_STEMS:
